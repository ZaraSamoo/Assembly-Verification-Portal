...
export default function PrincipalPortal() {
    ...
    try {
      ...
      if (profData) {
        ...
        if (instId) {
          ...
          // 2. Fetch 30-day submissions for institution_id from submissions table
          const querySubmissions: any = supabase.from('submissions');
          const { data: subData, error: subError } = await querySubmissions
            .select('*')
          ...
          if (!subError && subData) {
            historyRecords = subData as SubmissionRecord[];
          } else {
            // Fallback for assembly_submissions table if schema differs
            const queryAssemblySub: any = supabase.from('assembly_submissions');
            const { data: assemblyData } = await queryAssemblySub
              .select('*')
              .eq('campus_id', instId)
              .gte('created_at', thirtyDaysAgoIso)
              .order('created_at', { ascending: false });
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // 3. Insert row into submissions table referencing institution_id
      let insertedRecord: SubmissionRecord | null = null;

      const querySubmissions: any = supabase.from('submissions');
      const { data: subInserted, error: subErr } = await querySubmissions
        .insert({
      ...
      if (!subErr && subInserted) {
        insertedRecord = subInserted as SubmissionRecord;
      } else {
        // Fallback insert to assembly_submissions if submissions table does not match
        const queryAssembly: any = supabase.from('assembly_submissions');
        const { data: assInserted } = await queryAssembly
          .insert({
            campus_id: instId,
